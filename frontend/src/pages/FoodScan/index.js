...
const ScannedImg = () => {
  ...
  const handleSelectItem = (foodItem) => {
    if (selectedItems.find((item) => item.id === foodItem.id)) {
      setselectedItems(selectedItems.filter((item) => item.id !== foodItem.id))
    } else
      setselectedItems([...selectedItems, { ...foodItem, qty: 50, unit: 'gm' }])
  }
  ...
  useEffect(() => {
    if (!state.image_url || !state.foodItems || !state.imageBlob) navigate('/')
    else {
  ...
  return (
    <div maxWidth="lg" style={{ backgroundColor: 'var(--backgroundColor)' }}>
            ...
            <div>
              ...
              <div style={{ display: 'inline' }}>
                <input
                  ...
                  style={{ display: 'none' }}
                  onChange={(e) => {
                    if (e.target.files) handleSelectImage(e.target.files[0])
                  }}
                />
                <div
                  style={{
                ...
                >
                  ...
                  >
                    <CameraswitchIcon />{' '}
                    <span style={{ marginLeft: '7px' }}>Retake</span>
                  </label>
                </div>
              </div>
